...
def issue_refund(order: Order, amount_cents: int, reason: str, idempotency_key: str) -> str:
    ...
    `idempotency_key` is generated by the caller (agent_loop.py, the domain
    core) -- this adapter never generates it itself, only forwards it to
    Stripe so a repeated call for the same key is recognized as a duplicate
    and does not issue a second real refund."""
